...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Şifre değiştikten sonra kullanıcıyı otomatik olarak oturum açtır.
      // index.html /me endpointine Bearer token gönderdiği için access + refresh
      // tokenlarını doğrudan frontend'e döndürüyoruz.
      const loginResult = await client().auth.signInWithPassword({
        email: String(userData.user.email || entry.email || "").trim(),
        password
      });

      if (loginResult?.error || !loginResult?.data?.session?.access_token) {
        console.error(
          "RESET AUTO LOGIN ERROR:",
      ...
      const loggedUser = loginResult.data.user || userData.user;
      passwordResetTokens.delete(resetToken);
