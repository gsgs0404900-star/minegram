...
/* =========================================================
   MINEGRAM SMTP'SİZ ADMIN E-POSTA SERVİSİ EKİ
   Mevcut kodlar silinmeden eklenmiştir.
   Firebase Authentication'ın kendi e-posta gönderim altyapısını
   kullanır; SMTP kullanıcı adı/şifre gerekmez.
   ========================================================= */
const MINEGRAM_ADMIN_TEST_EMAIL = "minegramdestek@gmail.com";
...
app.post("/api/admin/email-service/config", async (req, res) => {
  // SMTP alanları geriye dönük uyumluluk için kabul edilir ancak saklanmaz.
  res.json({
    ok: true,
    configured: true,
    mode: "firebase-native",
    message: "SMTP ayarları gerekmiyor. Firebase Authentication e-posta servisi kullanılacak."
  });
});
...
app.post("/api/admin/email-service/verification-config", async (req, res) => {
  res.json({
    ok: true,
    codeLength: Number(req.body?.codeLength) === 8 ? 8 : 6,
    expiryMinutes: Number(req.body?.expiryMinutes) || 10,
    cooldownSeconds: Number(req.body?.cooldownSeconds) || 60,
    mode: "firebase-native"
  });
});
...
app.post("/api/admin/email-service/test", async (req, res) => {
  try {
    ...
    if (!idToken) {
      return res.status(401).json({
        ok: false,
        error: "Firebase admin oturumu gerekli."
      });
    }

    const firebaseUser = await firebaseLookupIdToken(idToken);
    ...
    if (uid !== "QJqw9moQk8XgpHcFo89bAVPk3uh1") {
      return res.status(403).json({
        ok: false,
        error: "Bu işlem yalnızca Minegram admin hesabı için kullanılabilir."
      });
    }

    if (email !== MINEGRAM_ADMIN_TEST_EMAIL) {
      return res.status(400).json({
        ok: false,
        error: `Admin Firebase hesabının e-postası ${MINEGRAM_ADMIN_TEST_EMAIL} olmalı. Mevcut hesap: ${email || "e-posta yok"}`
      });
    }
    ...
    return res.json({
      ok: true,
      sent: true,
      to: MINEGRAM_ADMIN_TEST_EMAIL,
      message: `Firebase doğrulama e-postası ${MINEGRAM_ADMIN_TEST_EMAIL} adresine gönderildi.`
    });
  } catch (e) {
    console.error("ADMIN FIREBASE MAIL TEST ERROR:", e?.message || e);
    return res.status(500).json({
      ok: false,
