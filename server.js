  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        const authUser =
          await resolveAuthUserForProfile(
            profile,
            admin
          );
...
async function resolveAuthUserForProfile(profile, admin) {
  ...
  if (profileEmail) {
    try {
      const { data, error } =
        await admin.auth.admin.getUserByEmail(profileEmail);
      if (!error && data?.user) return data.user;
    } catch {}
  }
...
async function resolveRecoveryEmail(identifier, mode = "email") {
  ...
  if (!raw) return null;
  const admin = SUPABASE_SERVICE_ROLE_KEY ? adminClient() : null;
  if (!admin) return null;

  if (mode === "username" || !raw.includes("@")) {
    const username = normalizeUsername(raw);
    try {
      const { data: profile, error } = await admin.from("profiles").select("*").eq("username", username).maybeSingle();
      if (!error && profile) {
        const authUser = await resolveAuthUserForProfile(profile, admin);
        if (authUser?.email) return { email: authUser.email, profile, authUser };
        if (profile.email) {
          const r = await admin.auth.admin.getUserByEmail(String(profile.email).trim().toLowerCase());
          if (r?.data?.user) return { email: r.data.user.email, profile, authUser: r.data.user };
        }
      }
    } catch (e) { console.error("USERNAME ACCOUNT LOOKUP ERROR:", e?.message || e); }
  }
  ...
  if (raw.includes("@")) {
    ...
    try {
      ...
      if (!authError && authData?.user) {
        ...
        let profile = null;
        try { profile = (await admin.from("profiles").select("*").eq("id", authUser.id).maybeSingle()).data || null; } catch {}
        if (!profile) {
          try { profile = (await admin.from("profiles").select("*").eq("auth_user_id", authUser.id).maybeSingle()).data || null; } catch {}
        }
        return { email: authUser.email, profile, authUser };
      }
    } catch (e) { console.error("EMAIL ACCOUNT LOOKUP ERROR:", e?.message || e); }
    try {
      const { data: profile, error } = await admin.from("profiles").select("*").eq("email", email).maybeSingle();
      if (!error && profile) {
        const authUser = await resolveAuthUserForProfile(profile, admin);
        if (authUser?.email) return { email: authUser.email, profile, authUser };
      }
    } catch (e) { console.error("PROFILE EMAIL LOOKUP ERROR:", e?.message || e); }
  }
  return null;
}
