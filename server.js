...
/* =========================================================
   REGISTER - SUPABASE AUTH E-POSTA OTP
   Resend yok. SMTP yok. Gmail SMTP yok.
   Supabase Auth 6 haneli OTP e-postasını kendi servisiyle gönderir.
========================================================= */
...
app.post("/api/register", async (req,res)=>{
  try{
    const username=normalizeUsername(req.body?.username), email=normalizeEmail(req.body?.email), password=String(req.body?.password||""), displayName=String(req.body?.displayName||username).trim().slice(0,80);
    ...
    const admin=adminClient();
    const {data:ep,error:epErr}=await admin.from("profiles").select("id,username").eq("username",username).limit(1).maybeSingle();
    if(epErr)throw epErr;if(ep)return res.status(409).json({ok:false,code:"USERNAME_TAKEN",error:"Bu kullanıcı adı zaten alınmış."});
    try{for(let page=1;page<=20;page++){const r=await admin.auth.admin.listUsers({page,perPage:1000});if(r?.error)break;const users=r?.data?.users||[];if(users.some(u=>normalizeEmail(u?.email)===email))return res.status(409).json({ok:false,code:"EMAIL_TAKEN",error:"Bu e-posta adresi zaten kullanılıyor. Giriş yap veya şifremi unuttum seçeneğini kullan."});if(users.length<1000)break;}}catch(e){}
    const {error:otpError}=await client().auth.signInWithOtp({email,options:{shouldCreateUser:true,data:{username,display_name:displayName}}});
    if(otpError)throw otpError;
    pendingRegistrations.set(email,{email,password,username,displayName,expires:Date.now()+10*60*1000});
    return res.json({ok:true,needsEmailVerification:true,nativeEmailVerification:true,otp:true,maskedEmail:maskEmail(email),email,user:{email,username,displayName},message:`6 haneli doğrulama kodu ${email} adresine gönderildi.`});
  }catch(e){console.error("REGISTER SUPABASE OTP ERROR:",e);return res.status(400).json({ok:false,error:e?.message||"Doğrulama e-postası gönderilemedi."});}
});
...
app.post("/api/register/verify",async(req,res)=>{
  try{
    ...
    if(!pending||pending.expires<Date.now()){pendingRegistrations.delete(email);return res.status(400).json({ok:false,code:"CODE_EXPIRED",error:"Kodun süresi dolmuş. Yeni kod iste."});}
    const {data:verified,error:verifyError}=await client().auth.verifyOtp({email,token:code,type:"email"});
    if(verifyError)throw verifyError;
    if(!verified?.session?.access_token||!verified?.user?.id)throw new Error("E-posta doğrulama oturumu oluşturulamadı.");
    const authed=client(verified.session.access_token);
    const {data:updated,error:updateError}=await authed.auth.updateUser({password:pending.password,data:{username:pending.username,display_name:pending.displayName}});
    if(updateError)throw updateError;
    const admin=adminClient();
    const existing=await admin.from("profiles").select("*").eq("id",verified.user.id).maybeSingle();
    if(existing.data){const r=await admin.from("profiles").update({username:pending.username,display_name:pending.displayName,auth_user_id:verified.user.id}).eq("id",verified.user.id);if(r.error)throw r.error;}
    else{const r=await admin.from("profiles").insert({id:verified.user.id,auth_user_id:verified.user.id,username:pending.username,display_name:pending.displayName,bio:"",avatar_url:null,verified:false,settings:{}});if(r.error&&r.error.code!=="23505")throw r.error;}
    pendingRegistrations.delete(email);
    return res.json({ok:true,verified:true,token:verified.session.access_token,nativeEmailVerification:true,user:{id:updated?.user?.id||verified.user.id,email,username:pending.username,displayName:pending.displayName}});
  }catch(e){console.error("REGISTER SUPABASE OTP VERIFY ERROR:",e);return res.status(400).json({ok:false,error:e?.message||"Kod doğrulanamadı."});}
});

app.post("/api/register/resend",async(req,res)=>{try{const email=normalizeEmail(req.body?.email);if(!email)return res.status(400).json({ok:false,error:"E-posta gerekli."});const pending=pendingRegistrations.get(email);if(!pending)return res.status(404).json({ok:false,error:"Bekleyen bir kayıt bulunamadı."});const {error}=await client().auth.signInWithOtp({email,options:{shouldCreateUser:true,data:{username:pending.username,display_name:pending.displayName}}});if(error)throw error;pending.expires=Date.now()+10*60*1000;pendingRegistrations.set(email,pending);return res.json({ok:true,nativeEmailVerification:true,otp:true,maskedEmail:maskEmail(email),message:"Yeni 6 haneli doğrulama kodu gönderildi."});}catch(e){return res.status(400).json({ok:false,error:e?.message||"Doğrulama e-postası yeniden gönderilemedi."});}});
...
/* =========================================================
   FORGOT START - SUPABASE NATIVE PASSWORD RESET
========================================================= */
