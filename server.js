...
async function auth(req, res, next) {
  try {
    ...
    /*
     * Token doğrulaması kullanıcı token'ı ile yapılır.
     * Profil okuması ise SERVICE ROLE ile yapılır. Böylece
     * profiles tablosundaki RLS politikaları /api/me'yi
     * yanlışlıkla "Oturum gerekli" durumuna düşürmez.
     */
    ...
    if (userError || !user?.id) {
      console.error(
        "AUTH TOKEN ERROR:",
        userError?.message || "Geçersiz oturum"
      );
    ...
    if (!profile) {
      ...
      if (byAuthId.error) {
        profileError = byAuthId.error;
      } else {
        profile = byAuthId.data || null;
      }
    }

    /*
     * Bazı eski Minegram profillerinde auth_user_id olmayabilir.
     * Bu durumda profile.id kullanılır.
     */
    if (!profile) {
    ...
    if (!profile) {
      ...
      return res.status(404).json({
        ...
        error: "Bu hesap için Minegram profili bulunamadı."
      });
    }
...
/*
 * Supabase Auth e-posta doğrulamasını güvenli biçimde tamamlar.
 * updateUserById başarılı olduktan sonra Auth kaydını tekrar okuyup
 * email_confirmed_at alanının gerçekten dolduğunu kontrol eder.
 * Böylece doğrulama isteği ile hemen sonraki login isteği arasında
 * oluşabilecek kısa süreli yayılım gecikmesi "Email not confirmed"
 * hatasına dönüşmez.
 */
async function confirmAuthEmail(admin, userId) {
  if (!admin || !userId) {
    throw new Error("Supabase Auth kullanıcı bilgisi eksik.");
  }
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    if (updateError) {
      ...
      if (!checkError && checkedData?.user?.email_confirmed_at) {
  ...
  throw lastError || new Error(
    "Supabase Auth e-posta doğrulaması doğrulanamadı."
  );
}
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      /*
       * Kod doğru: Supabase Auth e-postasını doğrula ve gerçekten
       * email_confirmed_at oluştuğunu kontrol et.
       */
      let confirmedUser;

      try {
        confirmedUser = await confirmAuthEmail(
          admin,
          entry.userId
        );
      } catch (confirmError) {
      ...
      /*
       * Şifre gönderilmediyse doğrulama yine başarılıdır.
       * Frontend normal giriş ekranından devam edebilir.
       */
      if (!password) {
        return res.json({
          ok: true,
          ...
          user: {
            id: confirmedUser?.id || entry.userId,
            email: entry.email,
            username: entry.username,
      ...
      /*
       * Doğrulama sonrası login denemesinde Supabase tarafında çok kısa
       * bir yayılım gecikmesi olabilirse birkaç kez tekrar dene.
       * Böylece kullanıcı doğrulama yaptığı halde "Email not confirmed"
       * mesajını görmez.
       */
      let loginData = null;
      let loginError = null;
      ...
      /*
       * E-posta doğrulaması tamamlandı. Login için gereken şifre yanlışsa
       * doğrulamayı geri alma; kullanıcı normal giriş ekranından tekrar
       * giriş yapabilsin.
       */
      return res.json({
        ok: true,
